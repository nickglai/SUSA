...
For example, if we have some temperatures in Celsius
...
Instead of adding a vector, beta, to alpha, where
```{r}
beta <- c(273.15, 273.15, 273.15, 273.15, 273.15, 273.15)
```
we can write
```{r}
alpha + 273.15 #which is the same value as
alpha + beta #but is much easier to write
```
...
What are 2 ways that we could manipulate vector gamma into vector delta?
```{r}
